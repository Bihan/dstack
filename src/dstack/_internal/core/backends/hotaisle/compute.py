    ...
    def update_provisioning_data(
        ...
        if vm_state == "running":
            if provisioning_data.hostname is None and provisioning_data.backend_data:
                ...
                # Start shim installation using daemon thread
...
def _start_runner(
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".pem", delete=False) as ssh_key_file:
        ssh_key_file.write(project_ssh_private_key)
        ssh_key_file.flush()
        _setup_instance(
            hostname=hostname,
            ssh_private_key_path=ssh_key_file.name,
        )
        _launch_runner(
            hostname=hostname,
            ssh_private_key_path=ssh_key_file.name,
...
def _launch_runner(
    ...
    # The launch_command already contains proper shim installation with nohup
    # But we need to add output redirection and disown for better daemonization
